...
var routes = function() {
  ...
  apiRouter
    ...
    .post(function(req, res) {
      if (!req.body.product) res.status(400).send("Need a product");
      var product = req.body.product;
      if (!product.productID) res.status(400).send("Need a productID");
      var productID = parseInt(product.productID);
      if (!req.body.quantity) res.status(400).send("Need a quantity");
      var quantity = parseInt(req.body.quantity);
      var cart = req.session.cart || [];
      var existingLines = cart.filter(function(cartLine) {
        return cartLine.product.productID == productID;
      });
      if (existingLines.length) existingLines[0].quantity += quantity;
      else
        cart.push({
          product: product,
          quantity: quantity
